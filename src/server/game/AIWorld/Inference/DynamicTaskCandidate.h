...
// handed to ValidateDynamicTaskCandidate() (see DynamicTaskValidation.h)
// via AIWorldMgr::OnDynamicTaskCandidateAccepted(). Nothing here
// authorizes gameplay: no Creature*/Player*/Unit*/Map*/Quest*, and that
// handoff itself does nothing but validate and log - no ActionRequest,
// no ActionSystem/ActionExecutor call, no DB write, no reward, no world
// mutation. RequestContext/Provenance are exactly what the originating
// PendingDynamicTaskRequest held, so the validator can re-derive
// everything about the request this draft answers without re-deriving
// live world state itself - live facts it cannot derive this way (the
// current source memory's payload, the current live target) are instead
// passed in separately as DynamicTaskAuthoritativeFacts.
