...
namespace LivingRecoveryPath
{
        ...
        LivingReturnPolicy::Diagnostics* diagnostics)
    {
        ...
        auto const& to = request.Destination;
        ...
        if (from.MapId != 0 || creature.GetZoneId() != 12 || creature.IsInCombat() || creature.IsInEvadeMode() ||
            ...
            !creature.IsAlive() || !UsefulStep(from, to) || Distance(from, to) > 30.0f ||
        ...
        if (request.Rejoin)
        {
            auto projected = RejoinPosition(creature);
            if (!creature.CanWalk() || !projected || Distance(*projected, to) > 1.0f) return reject("REJOIN_CHANGED");
        ...
        if (!Movement::PathWithinBounds(points, [&](float x, float y, float z)
            { return map->GetZoneId(creature.GetPhaseMask(), x, y, z) == 12 &&
                std::hypot(x - request.Home.X, y - request.Home.Y) <= request.HomeRadius; })) return reject("PATH_BOUNDS");
