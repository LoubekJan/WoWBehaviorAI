...
**Pilot vlčích smeček (2026-09-22): základní cyklus i pomoc smečky ve hře potvrzeny uživatelem.**
První viditelný cyklus propojuje teritorium → hlad → lov → krmení → spánek; zdraví vlci se brání, zranění utíkají. Rozsah je konfigurací vybraný druh WolfLoose (entry 69), nikoli všichni živočichové. Po opravě kořisti z entry 525 na 721 (Rabbit) uživatel potvrdil úspěšný herní test popsaného cyklu lovu, krmení a odpočinku. Původní cíl 525 blokovala přátelská herní reakce mezi vlky. Následný test potvrdil individuální obranu napadeného lovce; doplněna je pomoc blízkých členů stejné smečky (30 yardů, viditelnost spojence i útočníka, skutečný probíhající boj, přednost útěku při zranění). Uživatel následně potvrdil i úspěšný opakovaný test pomoci smečky při napadení jednoho lovícího vlka. [Implementace, lokální testy a runtime acceptance](doc/LivingWolves.md). Lokálně prošlo 70 samostatných C++ kontrol. Po hlášení překrývajících se členů byla doplněna stabilní místa při přesunu, oddělený příchod ke kořisti a různé směry pronásledování při lovu i obraně; ve výchozí konfiguraci místa členů 2–5členné smečky cílí na rozestup alespoň 3 yardy. Uživatel potvrdil základní funkčnost rozestupů, ale při otočení hráče přeživší vlci obíhali za jeho záda. Lokální oprava váže směry příchodu i pronásledování na světové souřadnice a samotné natočení cíle nepřepočítává cestu; běžné pronásledování ostatních NPC zachovává výchozí chování. S Catch2 v2.13.9 prošlo 432 kontrol v pěti testech včetně původních 211; uživatel následně potvrdil i funkční opravu otáčení ve hře. Kompletní výpis buildu a samostatné ověření ostatních regresních scénářů nebyly dodány. Plný lokální build blokuje chybějící Boost. Hranice dosahu/viditelnosti pomoci, oddělení různých smeček, přerušení krmení/spánku, unload/rebind a ostatní regresní scénáře zatím nemají samostatné runtime potvrzení.

**Lokální rozšíření rolí (22. 9. 2026):** Na potvrzený vlčí pilot navazuje implementace místních činností pro již řízená, klasifikovaná permanentní NPC v Elwynnu. Predátoři loví napadnutelnou kořist; kořist se toulá, pase a utíká; stráže a bojovníci se brání a pomáhají blízkým spojencům; civilisté, pracovníci, cestovatelé a služby mají odlišné místní činnosti. Připravené denní rutiny a ověřený vlčí cyklus si zachovávají prioritu. Rozšíření nemění účast spawnů ani oblasti mimo Elwynn. Deploy konfigurace zapíná `AIWorld.LivingRolesEnabled`; `.aiworld group status` vypisuje roli a aktuální činnost. Prošlo 672 assertions v 12 komponentových testech a syntaktická kontrola běhového kódu MSVC; plný lokální build blokuje chybějící Boost; následné runtime ověření rolí je zaznamenáno níže. Podrobný rozsah, omezení a postup ověření: [doc/LivingRoles.md](doc/LivingRoles.md).

**Runtime zpětná vazba a lokální oprava rolí (22. 9. 2026):** Uživatel potvrzuje ostatní základní činnosti, ale Forest Spider s hladem 1.00 nelovil Cow/Deer/Fawn a blízcí Defias nepomáhali ani s vypnutým GM režimem. Audit native DBC odhalil neutralitu pavouka FT 22 vůči kořisti a také vzájemnou neutralitu Defias Thug (FT 7 / klon 2300). Lokální oprava přidává omezený ekologický vztah řízený predátor → neutrální klasifikovaná divoká kořist pouze v Elwynnu a umožňuje pomoc stejné WorldFaction i bez native přátelství. Explicitní native nepřátelství mezi spojenci, ochrany hráčů, petů a služeb i běžné attackability kontroly zůstávají zachované. Pomoc načítá také live combat reference a scan běží při výchozí kadenci přibližně každou sekundu. Lov zkouší další kořist, pokud k nejbližší nevede cesta. Diagnostika přidává `lastHunt`/`lastAssist` a počty z posledního scanu. Prošlo 698 assertions v 14 komponentových testech a syntax runtime kódu; plný build včetně `Object.cpp` zde blokuje chybějící Boost. Uživatel následně 22. 9. 2026 výslovně potvrdil oba opakované herní testy jako funkční: lov Forest Spider i pomoc blízkých Defias při napadení spojence mají RUNTIME PASS pro tyto základní scénáře. Ostatní základní chování potvrdil už při prvním testu. Hraniční případy a výkon celé populace tím nejsou samostatně ověřené; nevznikla nová SQL/DBC migrace.

**Rozšíření vnímání a spolupráce (23. 9. 2026, LOCAL / RUNTIME PENDING):** `AIWorld.LivingRoleExtensionsEnabled` přidává řízeným rolím Elwynnu reakci před prvním zásahem, dosažitelné úkryty u stráží/domova, místní poplachy a prověřování stráží, krátkou paměť nebezpečných tras, stáda/dvojice stráží s rozestupy, stabilní individuální opatrnost, výběr kořisti podle vzdálenosti/zdraví/druhu a hovor s přítomným sousedem. Dokončená práce nově přidává omezené osobní zásoby Food/Resource jednou za pracovní okno; místní jídlo spotřebuje vlastní Food, pokud je k dispozici. Používá existující verzovanou persistenci bez migrace. Zásoby nejsou inventářové předměty ani kompletní tržní ekonomika, místní družení není nové persistentní členství v AgentGroup. Původní vlčí pilot a připravené rutiny pokračují. Prošlo **1 012 assertions v 21 komponentových testech**, syntax skutečných `LivingRole.cpp`, `ActionExecutor.cpp` a `AgentRegistry.cpp` pod MSVC a `git diff --check`. Plný build zde dál blokuje chybějící úplný Boost; nové chování čeká na herní test. [Tabulka rozšíření, hranice a sedm herních scénářů](doc/LivingRoles.md#rozšíření-okolí-spolupráce-a-zásob--23-9-2026). Rozšíření samo neuzavírá datový gate Etapy 3 ani dlouhodobou simulaci Etapy 4.

**Zpětná vazba k rozšíření a oprava lovu (23. 9. 2026, PARTIAL RUNTIME / HUNT RETEST PENDING):** Uživatel potvrdil výrobu 2 Resource během práce dřevorubce a reakci stráží po zásahu civilisty. Stádo hodnotí kladně, dvojice stráží předběžně také; u prasete je doložen stav úniku, nikoli kvalita trasy. Pavouk podle upřesnění celou dobu stojí; staré `HUNT_STARTED` s `IDLE` a bez akce neukazovalo důvod ukončení. Lokální oprava sjednocuje samostatný lov na přímé pronásledování kořisti místo pevného formačního místa, rozpoznává neprůchodnou cestu či chybějící chase a krátce vynechává nedosažitelnou kořist. Obrana a skupinové formace zůstávají. Diagnostika uchovává skutečný poslední důvod ukončení i aktuální engine údaje o pohybu a cíli. Prošlo **1 046 assertions ve 23 testech** a syntax běhového kódu pod MSVC; plný serverový build zde dál blokuje Boost. Souvislost opraveného nesouladu s hlášeným stáním ani výsledný pohyb zatím nemají runtime potvrzení. [Opakovaný test](doc/LivingRoles.md#zpětná-vazba-a-oprava-pronásledování--23-9-2026).

**Lov skutečně běží; vyvážení dostihnutí (23. 9. 2026, PURSUIT RUNTIME PASS / SPRINT RUNTIME PENDING):** Uživatel po opravě potvrdil rozběhnutí Forest Spider za kořistí. Kořist podle pozorování běžela stejně rychle a unikla; diagnostika uvádí `HUNT_LEASH` (Cow spawn 79880, odstup 29,5 yardu, predátor 35,1 yardu od domova). Následující lokální úprava přidává samostatnému lovu +35 % rychlosti běhu na 10 s a zvyšuje limit od místa zahájení z 30 na 60 yardů. Bonus patří jen danému chase, nemění Unit/template rychlosti a nepřenáší se do obrany ani vlčího pilotu. Diagnostika nově zobrazuje skutečné rychlosti běžících cest obou zvířat i čas sprintu. Prošlo **1 082 assertions ve 26 testech**, syntax běhového kódu včetně `ChaseMovementGenerator.cpp` a `MotionMaster.cpp`; plný lokální build nadále blokuje Boost. Skutečné dostihnutí po této změně čeká na opakovaný herní test. [Test sprintu](doc/LivingRoles.md#sprint-při-lovu-a-vzdálenost-pronásledování--23-9-2026).
