...
function showDiagnostics(agent) {
  ...
  facts("movement-facts", [
    ...
    ["Návraty do navštíveného bodu", recovery?.backtracks],
    ["Navigace – důvod odmítnutí", recovery?.navigation?.failure],
    ["Navigace – podrobnost", recovery?.navigation?.detail],
    ["Povolená vzdálenost návratu", recovery?.navigation?.home_radius],
    ["Lokální AI – stav", role?.advice?.status],
    ["Lokální AI – požadavky / spuštění / domů", role?.advice ? `${role.advice.requests} / ${role.advice.started} / ${role.advice.home_success}` : null],
    ["Lokální AI – jídlo / nedostupnost", role?.advice ? `${role.advice.food_success} / ${role.advice.unavailable}` : null],
    ["Navigační síť / dlaždice start / cíl", recovery?.navigation ? `${yes(recovery.navigation.mesh)} / ${yes(recovery.navigation.start_tile)} / ${yes(recovery.navigation.end_tile)}` : null],
    ["Vzdálenost od polygonu start / cíl", recovery?.navigation ? `${fixed(recovery.navigation.start_distance)} / ${fixed(recovery.navigation.end_distance)} yd` : null],
    ["Terénní filtr / start / cíl", recovery?.navigation ? `${recovery.navigation.filter} / ${recovery.navigation.start_flags} / ${recovery.navigation.end_flags}` : null],
    ["Zotavení plaváním / připojením k síti", recovery?.navigation ? `${yes(recovery.navigation.swimming)} / ${yes(recovery.navigation.rejoin)}` : null],
