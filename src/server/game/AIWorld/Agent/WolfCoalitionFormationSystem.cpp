...
std::optional<WolfCoalitionProposal> WolfCoalitionFormationSystem::Propose(std::vector<WolfCoalitionCandidate> const& candidates, WolfCoalitionFormationConfig const& config) const
{
    ...
    // the caller's own iteration order - see this class's own header
    // comment for why every candidate is tried as seed here (STATIC
    // review P2 fix), not just sorted.front().
    ...
    for (std::size_t seedIndex = 0; seedIndex < sorted.size(); ++seedIndex)
    {
        WolfCoalitionCandidate const& seed = sorted[seedIndex];
        ...
        // config.MaxMembers is always >= 1 by the time it reaches here -
        // copied from AgentGroupPolicyConfig::LooseMaxMembers, which
        // AIWorldMgr::Initialize() already clamps to at least
        // AIWorld.LooseGroupMinMembers (itself clamped to at least 1) - so
        // MaxMembers - 1 never underflows.
        ...
        if (uint32(1 + neighbors.size()) < config.MinMembers)
            continue; // this seed cannot form a viable pack - try the next
